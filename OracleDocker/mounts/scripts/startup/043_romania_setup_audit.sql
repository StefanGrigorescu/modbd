...
BEGIN
    ...
    FROM USER_TABLES
    WHERE UPPER(TABLE_NAME) = 'ESHOP_ROMANIA_LOGS';

    IF table_exists = 0 THEN
        EXECUTE IMMEDIATE '
            CREATE TABLE ESHOP_ROMANIA_LOGS (
        ...
        DBMS_OUTPUT.PUT_LINE('Table ESHOP_ROMANIA_LOGS created.');
    ELSE
        DBMS_OUTPUT.PUT_LINE('Table ESHOP_ROMANIA_LOGS already exists.');
    END IF;
END;
/
...
BEGIN
    INSERT INTO ESHOP_ROMANIA_LOGS (message, message_type, created_by, created_at, current_user)
