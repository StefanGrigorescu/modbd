...
BEGIN 
    ...
    LOG_DEBUG(
        'eshop_muntenia/TRY_CREATE_SHARD: Trying to create shard ' || shard_name || 
        ' | Container = ' || SYS_CONTEXT('USERENV', 'CON_NAME') || 
        ' | Schema = ' || SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') || 
        ' | Connected as = ' || SYS_CONTEXT('USERENV', 'SESSION_USER'), 
        created_by
...
BEGIN
    ...
    TRY_CREATE_SHARD(
        ...
        '
        ALTER TABLE SLS_MY_WISHLIST_ITEMS CONSTRAINT fk_sls_my_wishlist_items_idnt_users FOREIGN KEY (customer_id) REFERENCES IDNT_Users(id);
        ALTER TABLE SLS_MY_WISHLIST_ITEMS CONSTRAINT fk_sls_my_wishlist_items_sls_products FOREIGN KEY (product_id) REFERENCES SLS_Products(id);
        ', 'muntenia_create_tables');
    ...
    TRY_CREATE_SHARD(
        ...
        '
        ALTER TABLE SLS_MY_SHOPPING_CART_ITEMS CONSTRAINT fk_sls_my_shopping_cart_items_idnt_users FOREIGN KEY (customer_id) REFERENCES IDNT_Users(id);
        ALTER TABLE SLS_MY_SHOPPING_CART_ITEMS CONSTRAINT fk_sls_my_shopping_cart_items_sls_products FOREIGN KEY (product_id) REFERENCES SLS_Products(id);
        ALTER TABLE SLS_MY_SHOPPING_CART_ITEMS CONSTRAINT chk_sls_my_shopping_cart_items_quantity_positive CHECK (quantity >= 0);
        ', 'muntenia_create_tables');
    ...
    TRY_CREATE_SHARD(
        ...
        ALTER TABLE SLS_ORDER_ITEMS ADD CONSTRAINT fk_sls_order_items_sls_products (product_id) REFERENCES SLS_Products(id); 
    ...
    LOG_INFORMATION('Finished creating tables.', 'muntenia_create_tables');
END;
/
