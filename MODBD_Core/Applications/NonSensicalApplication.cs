...
public sealed class NonSensicalApplication : IApplication
{
    public required TextFieldSm Name { get; init; } = TextFieldSm.From(nameof(NonSensicalApplication));
    ...
    public static NonSensicalApplication On(string table, Conditions whereConditions) => new()
    {
        WhereConditions = whereConditions,
        Sql = $"Select 1 FROM {table} \nWHERE " + string.Join("\n\tAND ", whereConditions.Select(c => c.Value.Sql)),
    };
    [SetsRequiredMembers] private NonSensicalApplication() { }
}
